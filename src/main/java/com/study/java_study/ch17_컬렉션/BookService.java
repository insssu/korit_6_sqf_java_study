...
public class BookService {                  // BookService를 싱글톤으로 사용하는 것
    ...
    public void run1() {
        System.out.println("도서 전체 정보를 요청합니다.");         // run2()가 요청 받고
        run2();
    }

    public void run2() {
        System.out.println("도서 전체 정보 요청을 받았습니다.");      // run3()에 정보를 넘겨주고
    ...
    public List<BookEntity> run3() {
        System.out.println("데이터 베이스에서 도서 전체 정보를 조회합니다.");       // 전체정보를 book 객체로 하나하나 바꿔줘야 함
        ...
        for(Map<String, Object> bookMap : bookDB) {
            ...
            );    // Object 데이터들을 다운캐스팅으로 바꿔줘야 하므로 (int), (String) 명시해줘야 한다
